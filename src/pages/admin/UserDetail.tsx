    ...
    useEffect(() => {
        const fetchUserHistory = async () => {
            try {
                ...
                if (error.response) {
                    setError(
                        `Lỗi: ${error.response.status} - ${error.response.data?.message || "Không thể tải lịch sử giao dịch"
                        }`
                    );
                } else {
                                ...
                                {expandedTransactions.has(transaction.transactionId) && (
                                    <div className="mt-4 pl-12">
                                        ...
                                        {transaction.tickets.length === 0 ? (
                                            <p className="text-gray-500 italic">Không có vé nào trong giao dịch này</p>
                                        ) : (
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {transaction.tickets.map((ticket, index) => (
                                                    <div key={index} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                                                        <div className="flex justify-between items-start mb-3">
                                                            <div className="flex-1">
                                                                ...
                                                                <p className="text-sm text-gray-600 mb-2">
                                                                    Khu vực: <span className="font-medium">{ticket.areaName}</span>
                                                                </p>
                                                        ...
                                                        <button
                                                            onClick={() => handleViewQRCode(ticket.ticketCode)}
